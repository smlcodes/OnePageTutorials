...
We use EntityManager and UsersRepository to create Querydsl queries.
```java
/ where entityManager is a JPA EntityManager
JPAQuery<?> query = new JPAQuery<Void>(entityManager);
```

To retrieve the **user** with the name **Bob** you would construct a query like this:

```java
QUser user = QUser.user;
User bob = queryFactory.selectFrom(user)
...
query.from(user, company);
```
And to use multiple filters use it like this

```java
queryFactory.selectFrom(user)
.where(user.name.eq(***"Bob"***), user.lastName.eq(***"Wilson"***));
```
Or like this
```java
queryFactory.selectFrom(user)
.where(user.name.eq(***"Bob"***).and(user.lastName.eq(***"Wilson"***)));
```
In native JPQL form the query would be written like this:

select user from User as user
where user.name = "Bob" and user.lastName = "Wilson"

If you want to combine the filters via "or" then use the following pattern

queryFactory.selectFrom(user)
.where(user.name.eq(***"Bob"***).or(user.lastName.eq(***"Wilson"***)));
